...
export const useActivitiesHook = () => {
  ...
  const updateActivity = async (callId, archive) => {
    ...
    try {
      await request.patch(`${process.env.BASE_URL}/activities/${callId}`, {
        is_archived: archive,
      }, false);
    } catch (err) {
      throw Error(err);
  ...
  const getCallById = async (callId) => {
    ...
    try {
      const call = await request.get(`${process.env.BASE_URL}/activities/${callId}`);
  ...
  const resetActivitiesToInitialState = async () => {
    ...
    try {
      await request.patch(
        `${process.env.BASE_URL}/reset`, {}, false
      );
    } catch (err) {
      throw Error(err);
    }
